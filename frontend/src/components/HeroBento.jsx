  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* 1. 메인 배너 (좌측 2열 너비 차지) */}
        <div className="md:col-span-2 bg-gradient-to-br from-emerald-100/80 to-teal-50/60 rounded-[2rem] p-6 md:p-8 flex flex-col md:flex-row items-center justify-between overflow-hidden relative group min-h-[350px] md:min-h-[400px] shadow-xs">
          
          {/* 텍스트 영역 (패딩 및 줄간격 최적화로 공간 점유율 확대) */}
          <div className="flex flex-col text-left space-y-3 md:max-w-[55%] z-10">
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 leading-[1.1] tracking-tight whitespace-pre-line">
              Summer{"\n"}Arrival of{"\n"}Outfit
            </h1>
            <p className="text-slate-500 text-xs md:text-sm leading-relaxed max-w-[90%]">
          ...
          {/* 데코 50% OFF 배지 */}
          <div className="absolute top-6 right-8 text-right z-10 opacity-30 md:opacity-100">
            <span className="text-5xl md:text-6xl font-black text-emerald-800/15 block leading-none">50%</span>
            <span className="text-[10px] font-black text-emerald-700 tracking-widest uppercase">OFF SALE</span>
          </div>

          {/* 누끼 컷 모델 이미지 (크기를 키우고 꽉 차 보이도록 배치 조정) */}
          <div className="w-full md:w-[45%] mt-6 md:mt-0 flex justify-center items-end h-[105%] absolute -bottom-1 right-4 md:right-8">
            <img
              src="https://images.unsplash.com/photo-1620799140408-edc6dcb6d633?auto=format&fit=crop&w=500&q=80"
              alt="Summer Outfit"
              className="max-h-[290px] md:max-h-[390px] object-contain drop-shadow-2xl group-hover:scale-104 transition-transform duration-500 ease-out"
            />
          </div>
        </div>
        ...
        >
          ...
          {/* 아동 모델 컷 (깨지지 않는 신규 Unsplash 이미지 적용 및 크기 상향) */}
          <div className="w-[110%] h-[75%] -ml-[5%] mt-4 flex justify-center items-end relative overflow-hidden">
            <img
              src="https://images.unsplash.com/photo-1622290319146-7b63df48a635?auto=format&fit=crop&w=400&q=80"
              alt="Fashion Style Kids"
              className="max-h-[280px] md:max-h-[360px] object-contain drop-shadow-xl group-hover:scale-104 transition-transform duration-500 ease-out"
            />
          </div>
        </Link>
        ...
        >
          ...
          {/* 상품 이미지 (패딩 축소 및 오버랩 확대로 빈 공간 삭제) */}
          <div className="absolute right-0 bottom-0 w-[60%] h-[68%] flex justify-end items-end">
            <img
              src="https://images.unsplash.com/photo-1572635196237-14b3f281503f?auto=format&fit=crop&w=400&q=80"
              alt="Sunglasses"
              className="object-contain max-h-[115px] drop-shadow-lg group-hover:scale-106 transition-transform duration-500 ease-out"
            />
          </div>
        </Link>
        ...
        >
          ...
          {/* 상품 이미지 (패딩 축소 및 오버랩 확대로 빈 공간 삭제) */}
          <div className="absolute right-0 bottom-0 w-[65%] h-[72%] flex justify-end items-end">
            <img
              src="https://images.unsplash.com/photo-1606107557195-0e29a4b5b4aa?auto=format&fit=crop&w=400&q=80"
              alt="Popular Shoes"
              className="object-contain max-h-[125px] -rotate-12 drop-shadow-lg group-hover:scale-106 group-hover:rotate-0 transition-all duration-500 ease-out"
            />
          </div>
        </Link>
